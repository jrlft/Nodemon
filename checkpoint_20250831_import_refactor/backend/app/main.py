...
@app.post("/nodes/upload-csv/", status_code=status.HTTP_201_CREATED, dependencies=[Depends(get_current_username)])
async def upload_csv(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    for i, row in enumerate(csv_reader):
        ...
        try:
            ...
            if not ip_address:
                errors.append(f"Linha {line_num}: ip_address em falta.")
                continue
            ...
            secondary_ip = row.get('secondary_ip')
            if secondary_ip == '0':
                secondary_ip = None
            ...
            node_data = {
                ...
                "vps_provider": row.get('vps_provider'),
                "network": row.get('network'),
                "secondary_ip": secondary_ip,
            }
            
            NodeBase(**node_data) # Validate data
            nodes_to_create.append(node_data)
            existing_ips.add(ip_address) # Avoid duplicates from the same file

        except ValidationError as e:
            errors.append(f"Linha {line_num}: Erro de validação - {e}")
        except Exception as e:
    ...
    for i in range(0, len(nodes_to_create), batch_size):
        batch_data = nodes_to_create[i:i + batch_size]
        ...
        db.commit()
        logging.info(f"Lote de {len(batch_data)} nós salvo no banco de dados.")

    return {
        "message": f"{nodes_added} nós adicionados com sucesso. {nodes_skipped} nós ignorados (já existentes).",
        "errors": errors
    }
