...
const CSVImportModal = ({ isOpen, onClose, analysis, onImport, credentials }) => {
    const [nodesToOverwrite, setNodesToOverwrite] = useState([]);
    const [isImporting, setIsImporting] = useState(false);

    useEffect(() => {
        if (analysis) {
            // Pre-select all duplicates by default
            setNodesToOverwrite(analysis.duplicate_nodes.map(n => n.ip_address));
        }
    }, [analysis]);

    const handleToggleOverwrite = (ip_address) => {
        setNodesToOverwrite(prev => 
            prev.includes(ip_address) ? prev.filter(ip => ip !== ip_address) : [...prev, ip_address]
        );
    };
    ...
    const handleImportClick = async () => {
        setIsImporting(true);
        const payload = {
            nodes_to_create: analysis.new_nodes,
            nodes_to_update: analysis.duplicate_nodes.filter(n => nodesToOverwrite.includes(n.ip_address))
        };

        try {
            const response = await fetch('/api/nodes/import-processed-nodes/', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': credentials },
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.detail || 'Falha ao executar a importação.');
            }

            const result = await response.json();
            alert(result.message);
            onImport(); // This will refresh the main dashboard
        } catch (error) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
            <div className="bg-gray-800 p-8 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                <h2 className="text-2xl font-bold mb-4 text-white">Análise de Importação CSV</h2>
                
                {analysis ? (
                    <div className="flex-grow overflow-y-auto">
                        <div className="space-y-4 text-gray-300">
                            <p><span className="font-semibold text-green-400">{analysis.new_nodes.length}</span> nós novos prontos para serem importados.</p>
                            <p><span className="font-semibold text-yellow-400">{analysis.duplicate_nodes.length}</span> nós duplicados (IP já existe).</p>
                            {analysis.errors.length > 0 && (
                                <div>
                                    <p className="font-semibold text-red-400">{analysis.errors.length} erros encontrados no arquivo:</p>
                        ...
                        {analysis.duplicate_nodes.length > 0 && (
                            <div className="mt-6">
                                <h3 className="font-semibold mb-2 text-white">Sobrescrever Nós Duplicados?</h3>
                                <p className="text-sm text-gray-400 mb-3">Selecione os nós que deseja atualizar com os dados do CSV. Os não selecionados serão ignorados.</p>
                                <div className="bg-gray-900/50 rounded-lg p-3 max-h-60 overflow-y-auto">
                                    {analysis.duplicate_nodes.map(node => (
                                        <label key={node.ip_address} className="flex items-center space-x-3 p-2 hover:bg-gray-700/50 rounded-md">
                                            <input 
                                                type="checkbox" 
                                                checked={nodesToOverwrite.includes(node.ip_address)}
                                                onChange={() => handleToggleOverwrite(node.ip_address)}
                                                className="w-4 h-4 text-indigo-600 bg-gray-700 border-gray-600 rounded focus:ring-indigo-500 shrink-0"
                                            />
                                            <span className="font-mono text-sm text-white">{node.ip_address}</span>
                    ...
                    <div className="text-center py-10 text-gray-400">A analisar o ficheiro...</div>
                )}

                <div className="flex justify-end space-x-4 mt-6 pt-4 border-t border-gray-700">
                    <button onClick={onClose} className="py-2 px-5 bg-gray-600 hover:bg-gray-500 rounded-md text-white font-semibold">Cancelar</button>
                    <button 
                        onClick={handleImportClick} 
                        disabled={!analysis || isImporting}
                        className="py-2 px-5 bg-indigo-600 hover:bg-indigo-500 rounded-md text-white font-semibold disabled:opacity-50 disabled:cursor-wait"
                    >
                        {isImporting ? 'A Importar...' : 'Executar Importação'}
                    </button>
                </div>
            </div>
        </div>
...
const Dashboard = ({ project, credentials, setSidebarOpen }) => {
    ...
    const handleRefresh = useCallback(() => {
        setDashboardMessage(null); // Clear message on refresh
        fetchNodes();
        fetchGlobalStat();
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            const response = await fetch('/api/nodes/upload-csv/analyze', {
                method: 'POST',
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.detail || 'Falha ao analisar o CSV.');
            }
    ...
    const handleImportFinished = () => {
        setImportModalOpen(false);
        setImportAnalysis(null);
        setDashboardMessage("Importação concluída! A atualização dos status dos nós foi iniciada em segundo plano e pode levar alguns minutos.");
        // Trigger the safe, managed refresh endpoint
        fetch('/api/nodes/trigger-refresh', { method: 'POST', headers: { 'Authorization': credentials } });
        // Perform an immediate refresh to show nodes with "Aguardando verificação" status
        handleRefresh();
    };
    ...
    const availableStatuses = ['all', ...new Set(nodes.map(n => n.status).filter(Boolean))];
        ...
        <main className="flex-1 p-4 sm:p-8 bg-gray-800 text-gray-100 overflow-y-auto">
            ...
            {dashboardMessage && (
                <div className="bg-green-500/20 text-green-300 p-4 rounded-lg mb-6 flex justify-between items-center">
                    ...
                    <button onClick={() => setDashboardMessage(null)} className="text-green-300 hover:text-white">
            ...
            <div className="mb-6 space-y-4">
                <div className="flex flex-col sm:flex-row gap-4">
                    <div className="relative flex-grow">
                        <input 
                            ...
                            className="w-full bg-gray-700 text-white p-3 pl-10 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        />
                    ...
                    <select 
                        ...
                        className="bg-gray-700 text-white p-3 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:w-auto"
                    >
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => handleOpenNodeModal()} className="flex items-center bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 px-4 rounded-lg text-sm"><Icon path="M12 4.5v15m7.5-7.5h-15" className="w-5 h-5 mr-2" />Adicionar Nó</button>
            ...
            <div className="flex items-center justify-end space-x-2 sm:space-x-4 mb-4">
                ...
                <span className="text-sm text-gray-400">
                    {indexOfFirstNode + 1}-{Math.min(indexOfLastNode, filteredNodes.length)} de {filteredNodes.length}
                </span>
